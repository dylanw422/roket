export async function jobFinder(page, browser, socket) {
  ...
  const jobCards = await page.locator("li[id^='ember']");
  ...
  for (let i = 0; i < 1; i++) {
    ...
    //START APPLICATION
    await page.locator(".jobs-apply-button.artdeco-button").first().click();

    socket.emit("process", `${jobTitle} @ ${companyName.trim()}`);

    // CLICK THROUGH AND FILL OUT APPLICATION FORM
